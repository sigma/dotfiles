...
;; $Id: nnir.el,v 1.1 2004/04/19 17:53:57 sigma Exp $
...
(defconst nnir-version "$Id: nnir.el,v 1.1 2004/04/19 17:53:57 sigma Exp $"
  "Version of NNIR.")
...
Tested with swish-e-2.0.1 on Windows NT 4.0."
  ...
  (save-excursion
    (let ( (qstring (cdr (assq 'query query)))
	   (prefix (nnir-read-server-parm 'nnir-swish-e-remove-prefix server))
           (artlist nil)
           (score nil) (artno nil) (dirnam nil) (group nil) )
      ...
      (let* ((cp-list `( ,nnir-swish-e-program
                         ...
                         t              ; output
                         nil            ; don't redisplay
                         "-f" ,(nnir-read-server-parm 'nnir-swish-e-index-file server)
                         ,@(nnir-read-server-parm 'nnir-swish-e-additional-switches server)
                         "-w"
