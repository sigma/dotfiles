...
;; Version: 22.0
;; Last-Updated: Tue Nov 15 09:25:59 2005 (-28800 Pacific Standard Time)
;;           By: dradams
;;     Update #: 12284
...
;;
;;   `bookmark', `cl', `color-theme', `cus-face', `easymenu',
;;   `hexrgb', `misc-fns', `pp', `recentf', `wid-edit', `widget'.
;;
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
;;
;;; Commentary:
...
;;    `icicle-completion-help', `icicle-delete-windows-on',
...
;;    `icicle-sort-function', `icicle-mark-position-in-candidate',
...
;;
;;    This idea of buffer-option configurations was borrowed from
...
;;    above.
;;
;;    [insert] - `icicle-switch-to-*Completions*': Move cursor to the
...
;;
;;  3. Extend the idea behind `icicle-candidate-set-define', to let
;;     users edit buffer *Completions* to redefine the completions
;;     list.
;;
;;  4. Consider doing the key binding & unbinding differently - copy
;;     standard bindings when enter `icicle-mode' and restore them
;;     when leave it?
;;
;;  5. Replace redefinition of `exit-minibuffer' and/or
...
This does everything, except deal with regexp-match errors."
  ...
  (cond ((null icicle-completion-candidates)
         (setq icicle-nb-of-other-cycle-candidates 0)
         (save-selected-window (when (get-buffer-window "*Completions*" 0)
                                 (delete-window (get-buffer-window "*Completions*" 0))))
  ...
  (let ((frame-with-focus (selected-frame))
    ...
    (cond (cand-symb
           (cond ((functionp cand-symb) (describe-function cand-symb))
                 ...
                 (t (call-interactively 'help-command))))
          (t                            ; Not a symbol - treat string itself.
           (cond ((and (bufferp (get-buffer icicle-last-completion-candidate))
                 ...
                 (t
                  (call-interactively 'icicle-completion-help)))))
...
regexp searching: 1) the `grep' regexp, 2) the major `icicle-search' regexp,
and optionally 3) the refining `icicle-search' regexp."
  (interactive
   (if (or (null (mark)) (= (point) (mark)) (not mark-active))
       (list (point-min) (point-max))
     (if (< (point) (mark))
...
(defun icicle-delete-windows-on (buffer) ; From `remove-windows-on' in `frame-cmds.el'.
  ...
  (when buffer                          ; Do nothing if null BUFFER.
    (dolist (fr (icicle-frames-on buffer t))
...
`minibuffer-local-must-match-map'."
  ...
  (define-key map [insert]                   'icicle-switch-to-*Completions*)
...
Usually run by inclusion in `minibuffer-setup-hook'."
  (cond ((and icicle-mode (window-minibuffer-p (selected-window)) (not executing-kbd-macro))
	 ...
	 (add-hook 'post-command-hook (function (lambda () (run-hooks 'icicle-post-command-hook)))
		   nil t)
