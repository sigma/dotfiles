...
;;; @(#) $Id: gnus-alias.el,v 1.1 2004/04/19 17:54:02 sigma Exp $
...
;;  an Identity given the message context.  You should alse set up
...
;;
;;  Finally, there are a whole slew of functions that mimic the basic
;;  gnus commands for initiating a message, only with these new ones
;;  you can pre-specify a particular Identity to use (bypassing all of
;;  the normal Identity selection logic).  They are:
;;
;;       <coming soon>
;;
;;  These functions can be bound to keys any way that you'd like, but
;;  there is a helper function that <is also coming soon>.
...
;;    signature, will also remove the forwarded message.  This will be
;;    fixed at some point.
...
;;    will probably not be fixed anytime soon.
;;  o It's possible for a loop to be created when having one Identity
;;    refer to another.  This will be fixed at some point.
...
;;  21.0.105 under Windows 2000, Emacs 20.7.1 under Windows NT 4.0 and
;;  Emacs 20.7.1 under Linux (RH7).  Please, let me know if it works
...
article or follow-up to am article or a forwarded email or article).
...
(defun gnus-alias-create-identity-menu ()
  "Adds Identity menu in Message mode (see `gnus-alias-add-identity-menu')."
