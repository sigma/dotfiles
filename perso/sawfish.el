...
(defcustom sawfish-program "sawfish"
  "*Window manager executable."
  :type  'string
  :group 'sawfish)
...
(defun sawfish-compile (file &optional target-buffer)
  (call-process sawfish-program nil target-buffer nil "--batch" "-l" "compiler" "-f" "compile-batch" file))

(defun sawfish-compile-this-file ()
  (interactive)
  (message (format "Compiling %s" (buffer-file-name)))
  (sawfish-compile (buffer-file-name)))
...
(easy-menu-define sawfish-mode-menu sawfish-mode-map "sawfish commands"
  '("Sawfish"
    ...
    "----"
    ["Byte-compile This File"          sawfish-compile-this-file t]
    "----"
