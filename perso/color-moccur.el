...
;; $Id: color-moccur.el,v 1.1 2004/06/15 16:27:57 sigma Exp $
...
(defcustom dmoccur-maximum-size 100
...
(defcustom moccur-kill-buffer-after-goto t
    ...
    (let ((total (length files))
      ...
      (condition-case err
          (while files
            (setq num (+ num 1))
            (with-temp-buffer
              (when (and (> (* 1000 dmoccur-maximum-size)
...
(defun moccur-grep-goto ()
  ...
  (let (file line)
    (save-excursion
      (if (re-search-backward "File(Grep): \\([^\r\n]+\\)$" nil t)
          (setq file
                (buffer-substring-no-properties
                 (match-beginning 1)
                 (match-end 1)))))
