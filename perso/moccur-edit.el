...
;;  $Id: moccur-edit.el,v 1.1 2004/06/15 16:27:57 sigma Exp $
...
;; moccur-edit provides to edit moccur buffer and to apply the changes
;; to the file.
...
;; "C-x C-q" or "r".
;; Use C-c C-f or C-x C-s or C-c C-c when finished
;;     C-x k or C-c C-k to abort
;;     C-c C-r to remove changes of region.
...
(defun moccur-edit-reset-key ()
  ...
  (if (string= mode-name "Moccure-edit")
      (moccur-mode t)
    (moccur-mode))
  (force-mode-line-update)
  )
...
(defun moccur-edit-mode ()
  "Major mode"
  (let ((ee (string= "Moccur-ee" mode-name)))
